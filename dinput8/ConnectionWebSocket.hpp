...
{
public:
	ConnectionWebSocket(ProxyType type, std::function<void(unsigned char*, unsigned int)> sendToGame, std::function<void()> closeCallback);
...
private:
	...
	void handle_disconnect(web::websockets::client::websocket_close_status close_status, const utility::string_t& reason, const std::error_code& error);
};