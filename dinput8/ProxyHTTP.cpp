...
{
	new_http_connection.reset(new HttpHandler((io_context&)acceptor_.get_executor().context()));
	acceptor_.async_accept(new_http_connection->gameSocket(), boost::bind(&ProxyHTTP::handle_accept, this, asio::placeholders::error));
}
